        ...
        testList "Trainer" [
            ...
            testList "categorize" [
               testCase "when category no exist in state" <| fun _ ->
                   let subject = (Config.Default(), State.empty()) |> Trainer.categorize({ value = ""; category = 2; weight = None }, ["cute"; "dog"])
               ...
               testCase "when category exist in state" <| fun _ ->
                   ...
                   let subject = (Config.Default(), state) |> Trainer.categorize({ value = ""; category = 2; weight = None }, ["cute"; "dog"])
                   Expect.isTrue (subject.categories.ContainsKey(2)) "should contain 2"
